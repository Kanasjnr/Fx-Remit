...
describe('FXRemitV2', function () {
  ...
  async function deployFXRemitV2Fixture() {
    ...
    const fromToken = (await MockERC20Factory.deploy(
      'Test Token A',
      'TTA'
    )) as any;
    const toToken = (await MockERC20Factory.deploy(
      'Test Token B',
      'TTB'
    )) as any;
    const intermediateToken = (await MockERC20Factory.deploy(
      'Test Token C',
      'TTC'
    )) as any;
    ...
    await fxRemit
      .connect(owner)
      .addSupportedToken(await fromToken.getAddress(), 'TTA');
    await fxRemit
      .connect(owner)
      .addSupportedToken(await toToken.getAddress(), 'TTB');
    await fxRemit
      .connect(owner)
      .addSupportedToken(await intermediateToken.getAddress(), 'TTC');
    ...
    await (fromToken as any)
      .connect(sender)
      .approve(await mockPermit2.getAddress(), ethers.MaxUint256);

    // Fund adapter with output tokens (for swap simulation)
    await (toToken as any)
      .connect(owner)
      .mint(owner.address, ethers.parseEther('10000'));
    await (toToken as any)
      .connect(owner)
      .transfer(await adapter.getAddress(), ethers.parseEther('10000'));

    await (intermediateToken as any)
      .connect(owner)
      .mint(owner.address, ethers.parseEther('10000'));
    await (intermediateToken as any)
      .connect(owner)
  ...
  describe('Deployment', function () {
    ...
    it('Should initialize with correct default adapter', async function () {
      ...
      expect(await fxRemit.defaultAdapter()).to.equal(
  ...
  describe('Token Management', function () {
    it('Should allow owner to add supported token', async function () {
      ...
      await expect(
        fxRemit
          .connect(owner)
          .addSupportedToken(await newToken.getAddress(), 'NEW')
      )
        .to.emit(fxRemit, 'TokenAdded')
        .withArgs(await newToken.getAddress(), 'NEW');

      expect(await fxRemit.isSupportedToken(await newToken.getAddress())).to.be
        .true;
      expect(
        await fxRemit.getTokenSymbol(await newToken.getAddress())
      ).to.equal('NEW');
    });

    it('Should prevent adding duplicate token', async function () {
      const { fxRemit, owner, fromToken } = await loadFixture(
        deployFXRemitV2Fixture
      );
    ...
    it('Should allow owner to remove supported token', async function () {
      ...
      await expect(
        fxRemit
          .connect(owner)
          .removeSupportedToken(await fromToken.getAddress())
      )
        .to.emit(fxRemit, 'TokenRemoved')
        .withArgs(await fromToken.getAddress());

      expect(await fxRemit.isSupportedToken(await fromToken.getAddress())).to.be
        .false;
    });

    it('Should prevent swap with unsupported token', async function () {
      const { fxRemit, sender, recipient, toToken, provider, exchangeId } =
        await loadFixture(deployFXRemitV2Fixture);
  ...
  describe('Adapter Management', function () {
    it('Should allow owner to propose new adapter', async function () {
      ...
      const newAdapter = (await MockAdapterFactory.deploy(
        await fxRemit.getAddress()
      )) as any;
      ...
      await expect(
        ...
        .withArgs(await newAdapter.getAddress(), (value: bigint) => {
          ...
          const diff =
            value > expectedTime
              ? value - BigInt(expectedTime)
              : BigInt(expectedTime) - value;
          return diff <= 2n;
        });
    });
    ...
    it('Should enforce timelock before activation', async function () {
      ...
      await fxRemit
        .connect(owner)
        .proposeSwapAdapter(await newAdapter.getAddress());

      await expect(
        fxRemit
          .connect(owner)
          .activateSwapAdapter(await newAdapter.getAddress())
    ...
    it('Should allow activation after timelock', async function () {
      ...
      expect(await fxRemit.isAllowedAdapter(await newAdapter.getAddress())).to
        .be.true;
    });

    it('Should prevent removing default adapter', async function () {
      const { fxRemit, owner, adapter } = await loadFixture(
        deployFXRemitV2Fixture
      );
  ...
  describe('Provider and Exchange Whitelisting', function () {
    it('Should allow owner to whitelist provider', async function () {
      const { fxRemit, owner, otherAccount } = await loadFixture(
        deployFXRemitV2Fixture
      );
    ...
    it('Should allow owner to whitelist exchange', async function () {
      ...
      await expect(fxRemit.connect(owner).setExchangeAllowed(exchangeId, true))
    ...
    it('Should prevent swap with unwhitelisted provider', async function () {
      const { fxRemit, sender, recipient, fromToken, toToken, exchangeId } =
        await loadFixture(deployFXRemitV2Fixture);
  ...
  describe('swapAndSend', function () {
    ...
    it.skip('Should execute single-hop swap successfully (requires Permit2 setup)', async function () {
      ...
      await fromToken
        .connect(sender)
        .approve(await fxRemit.getAddress(), amountIn);
      await fromToken
        .connect(sender)
        .transfer(await fxRemit.getAddress(), amountIn);
  ...
  describe('Fee Management', function () {
    it.skip('Should allow owner to withdraw collected fees (requires swap execution)', async function () {
      ...
      const { fxRemit, owner, toToken } = await loadFixture(
        deployFXRemitV2Fixture
      );
      ...
      await expect(
        fxRemit
          .connect(owner)
          .withdrawTokenFees(await toToken.getAddress(), owner.address)
  ...
  describe('Security', function () {
    it('Should prevent non-owner from pausing', async function () {
      const { fxRemit, otherAccount } = await loadFixture(
        deployFXRemitV2Fixture
      );
