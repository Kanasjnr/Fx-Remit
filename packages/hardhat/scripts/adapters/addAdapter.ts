...
async function main() {
  // Configuration
  const FX_REMIT_V2_ADDRESS =
    process.env.FX_REMIT_V2_ADDRESS || 'YOUR_CONTRACT_ADDRESS';
  const NEW_ADAPTER_ADDRESS =
    process.env.NEW_ADAPTER_ADDRESS || 'YOUR_ADAPTER_ADDRESS';
  ...
  // Get contract
  const fxRemitV2 = await ethers.getContractAt(
    'FXRemitV2',
    FX_REMIT_V2_ADDRESS
  );
  ...
  console.log(
    'To make it active, run: npx hardhat run scripts/adapters/setDefault.ts'
  );
}
