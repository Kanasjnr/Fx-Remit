...
async function main() {
  ...
  if (networkName !== 'celo') {
    console.error(
      `❌ This script only supports Celo mainnet deployment. Current network: ${networkName}`
    );
  ...
  try {
    ...
    console.log(
      `You can manually verify later: npx hardhat verify --network celo ${contractAddress}`
    );
  }
