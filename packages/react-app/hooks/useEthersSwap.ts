...
export function useEthersSwap() {
  ...
  const swap = async (
    ...
    console.log(`Quote: ${ethers.utils.formatEther(quoteAmountOut)} ${toCurrency} for ${amount} ${fromCurrency}`);
    console.log(`Expected amount out with 2% slippage: ${ethers.utils.formatEther(expectedAmountOut)} ${toCurrency}`);
    
    try {
      // 1) Find tradable pair - try multiple approaches
      let tradablePair = await mento.findPairForTokens(fromTokenAddress, toTokenAddress);
      ...
      if (!tradablePair) {
        ...
        throw new Error(`No tradable pair found for ${fromCurrency} → ${toCurrency}. This pair may not be supported by Mento exchanges.`);
      }
      ...
      if (tradablePair.path.length === 2) {
        ...
        if (!hop1.assets.includes(fromTokenAddress)) {
          ...
          throw new Error(`Multi-hop swap failed: The first exchange doesn't support ${fromCurrency}. This token pair may not be available for trading. Please try a different currency pair.`);
        }
        ...
        if (!hop2.assets.includes(toTokenAddress)) {
          ...
          throw new Error(`Multi-hop swap failed: The second exchange doesn't support ${toCurrency}. This token pair may not be available for trading. Please try a different currency pair.`);
        }
        
        // Find intermediate token
        const intermediateToken = hop1.assets.find(asset => 
        ...
        if (!intermediateToken) {
          ...
          throw new Error('No valid intermediate token found between the two hops');
        }
      ...
      const currentAllowance = await tokenContract.allowance(signerAddress, fxRemitAddress);
      
      if (currentAllowance.lt(amountInWei)) {
        console.log('Approving token spend...');
        const approvalTx = await tokenContract.approve(fxRemitAddress, amountInWei);
      ...
      if (tradablePair.path.length === 1) {
        ...
        const fxRemitContract = new Contract(fxRemitAddress, FXRemitABI, signer);
      ...
      if (tradablePair.path.length === 2) {
        ...
        try {
          ...
          if (multiHopError instanceof Error && multiHopError.message.includes('tokenIn and tokenOut must match exchange')) {
            throw new Error(`Multi-hop swap failed: The exchange parameters don't match the token pair. This might be due to exchange configuration issues. Please try a different token pair or contact support.`);
          }
